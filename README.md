...
为什么这里需要 64 位地址？首先，对于一个指针类型的全局变量而言，相对寻址是不适用的。其次，x86-64 的地址是 64 位的，而 32 位地址最多只能访问低 4GB 空间。如果程序被加载到高地址区域，32 位地址将无法访问到正确的内存。
